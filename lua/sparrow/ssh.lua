...
function M.ssh_error_retry(command, host)
...
function M.exec_on_host(command, run_host)
  local ssh_command = string.format(
    ...
    run_host.password,
    run_host.port,
    run_host.username.run_host.ip,
    command
  )

  local exit_code, output = M.ssh_error_retry(ssh_command, run_host)
  if exit_code ~= 0 then
    logger.error("exec ssh command(%s) failed, err:%s", ssh_command, output)
    return output
  end
