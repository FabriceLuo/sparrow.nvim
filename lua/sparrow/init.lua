...
function M.with_rule_commands(rules, callback)
  ...
  for i = 1, #rules do
    local r = rules[i]
    if r.pre_upload_commands ~= nil then
      pre_cmds_group.add_commands(r.pre_upload_commands)
    end

    if r.post_upload_commands ~= nil then
      post_cmds_group.add_commands(r.post_upload_commands)
    end
  end
...
function M.with_buf_rule(bufno, rule_opts, callback)
  local cur_rule = rule.get_buf_rule(bufno)
  if cur_rule == nil then
    rule.gen_buf_rule(bufno, rule_opts, function(buf_rule)
